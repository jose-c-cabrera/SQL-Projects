/*                                                                        */
/*              Instpub.SQL - Creates the pub database                  */ 
/*                                                                        */
/*
...
if exists (select * from sysdatabases where name='pub')
begin
...
if db_name() <> 'pub'
   raiserror('Error in Instpub.SQL, ''USE pub'' failed!  Killing the SPID now.'
            ,22,127) with log

GO

--execute sp_dboption 'pub' ,'trunc. log on chkpt.' ,'true'
...
declare @dttm varchar(55)
select  @dttm=convert(varchar,getdate(),113)
raiserror('Ending Instpub.SQL at %s ....',1,1,@dttm) with nowait
